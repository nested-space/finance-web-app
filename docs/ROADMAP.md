...
| D-012 | Make categories user-managed (table) and move the budget amount to the category level | Resolved — approved | **Reverses the fixed seven-value `Category` enum.** `Category` is now a user-managed table shared by budgets, budget items, expenses, and commitments (one list; the commitment category subset is dropped). The budget amount lives at the category level (a budget no longer has a name); new `budget_item` labels classify within a category and tag expenses (`expense.budget_item_id`). Category deletion is **blocked while in use**. Breaking schema + JSON-contract change (MAJOR at 1.0.0); supersedes the "enum member names are stored" invariant. No new dependency. |
